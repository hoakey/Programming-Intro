function gameLoop() {	
	...
	// STEP 3: Move the player paddle with the arrow keys. Use the following logic:
	// IF isUpPressed()
	...
	// STEP 4: Move AI with logic:
	// IF isBallBelowAIPaddle()
	//   THEN aiPaddle.speed = paddleSpeed
	// ELSE aiPaddle.speed = paddleSpeed * -1
	
	
	
	// STEP 5: Bounce ball off either paddle with logic:
	...
	// STEP 6: Bounce ball off top or bottom with logic:
	...
	// STEP 7: Check if the ball has scored on either side with logic:
	// IF ball.xPosition IS LESS THAN 0
	//   THEN playerScore = playerScore + 1 AND THEN startPlayer()
	// IF ball.xPosition IS GREATER THAN screenWidth
	//   THEN aiScore = aiScore + 1 AND THEN startPlay()
